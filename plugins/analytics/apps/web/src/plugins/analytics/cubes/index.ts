/**
 * Cube registry (D19): `allCubes()` is what `api/routes/cube-api.ts` hands to `createCubeApp` on
 * every request and what `tests/api/cube-isolation.test.ts` walks. Adding a cube = a file here +
 ...
 * through `analyticsExtensions({ cubes })` (D31 decision 6). A FUNCTION rather than a const
 * because reading `serverPlugins` at module scope here would close a cycle — the server barrel
 * imports this plugin's entry, which reaches this file — and because `contributedCubes` memoises,
 * so the walk is paid once per isolate.
 *
...
export function cubesFor(features: readonly string[]): Cube[] {
  ...
  return hidden.size === 0 ? all : all.filter(cube => !hidden.has(cube))
}
